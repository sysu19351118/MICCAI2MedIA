...
def create_block(
    ...
    # print(RMSNorm)
    mixer_cls = partial(Mamba, layer_idx=layer_idx, bimamba_type=bimamba_type, init_layer_scale=init_layer_scale, **ssm_cfg, **factory_kwargs)
    norm_cls = partial(
...
class VisionMambaencoder(VisionMamba):
    def __init__(self, img_size=224, patch_size=16, stride=16, depth=24, embed_dim=192, channels=3, num_classes=1000, ssm_cfg=None, drop_rate=0., drop_path_rate=0.1, norm_epsilon: float = 1e-5, rms_norm: bool = False, initializer_cfg=None, fused_add_norm=False, residual_in_fp32=False, device=None, dtype=None, ft_seq_len=None, pt_hw_seq_len=14, if_bidirectional=False, final_pool_type='none', if_abs_pos_embed=False, if_rope=False, if_rope_residual=False, flip_img_sequences_ratio=-1., if_bimamba=False, bimamba_type="none", if_cls_token=False, if_devide_out=False, init_layer_scale=None, use_double_cls_token=False, use_middle_cls_token=False):
        super().__init__()
        self.linear512 = nn.Linear(embed_dim, out_features=512)  # 添加一个线性层，将输出维度从embed_dim映射到512

        # self.image_encoder = VisionMamba(
        #             patch_size=16, 
        #             embed_dim=192,  # 保持模型原始的嵌入维度
        #             depth=24, 
        #             rms_norm=True, 
        ...
        #             num_classes=0 #设置num_classes=0
        #         )
        # self.image_projection_head = nn.Linear(self.image_encoder.embed_dim, 512)
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if self.final_pool_type == 'max':
            x = x.max(dim=1)[0]
        x=self.linear512(x)  # 将输出维度从embed_dim映射到512
        return x
