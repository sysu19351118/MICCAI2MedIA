...
try:
    from timm.models import _update_default_kwargs as update_args
except:
...
class MambaVisionEncoder(nn.Module):
    """
    一个包装 MambaVision 的图像编码器。
    - 加载预训练的 MambaVision 模型。
    - 移除原始的分类头。
    - 使用 forward_features() 提取特征。
    - 添加一个投影层，将特征维度映射到期望的输出维度 (512)。
    """
    def __init__(self, output_dim=512, pretrained=True, model_variant='S'):
        super().__init__()
        
        # 加载预训练的 MambaVision 模型
        # 这里你可以轻松切换不同的模型变体，如 mamba_vision_T, mamba_vision_B 等
        if model_variant == 'S':
            # 你可以提供本地路径，如果文件不存在，它会自动从URL下载
            self.mamba_vision = mamba_vision_S(pretrained=pretrained, model_path="/tmp/mamba_vision_S.pth.tar")
        elif model_variant == 'T':
            self.mamba_vision = mamba_vision_T(pretrained=pretrained, model_path="/tmp/mamba_vision_T.pth.tar")
        # ... 你可以添加更多模型的elif分支 ...
        else:
            raise ValueError(f"Unsupported MambaVision variant: {model_variant}")

        # 获取 MambaVision 模型在进入分类头之前的特征维度 (例如，'S'版本是768)
        feature_dim = self.mamba_vision.head.in_features
        
        # 将原始的分类头替换为 Identity，这样就不会执行分类操作
        self.mamba_vision.head = nn.Identity()

        # 添加一个新的线性层，将特征维度从 feature_dim 映射到你需要的 output_dim (512)
        self.projection = nn.Linear(feature_dim, output_dim)

    def forward(self, x):
        # 1. 使用 mamba_vision 提取特征
        # forward_features 的输出 shape: (batch_size, feature_dim)
        features = self.mamba_vision.forward_features(x)
        
        # 2. 通过投影层将维度调整为 512
        # projected_features 的输出 shape: (batch_size, 512)
